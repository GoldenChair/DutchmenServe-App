...
abstract class Repository {
  ...
  // Dont think these are needed client side
  // // add interest
...
class FakeRepository implements Repository {
  static final String url = **DELETED**
  ...
  //       id: 0, email: 'b@lvc.edu', officers: [1, 2, 3, 4, 5], members: [1, 2]),
  //   Organization('Colleges Against Cancer',
  //       'see me at Relay, this is a longer description',
  //       id: 1, email: 'c@lvc.edu', officers: [3], members: [3, 4, 5]),
  ...
  //       id: 3, email: 'ast@lvc.edu', officers: [7], members: [7, 10, 12]),
  //   Organization('Gamma Sigma Sigma', 'xmas we organize the presents',
  //       id: 4, email: 'gs@lvc.edu', officers: [], members: [11, 12]),
  // ];
  //INTERESTS
  ...
  Future<List<Event>> getEvents() async {
    ...
    try {
      var response =
          await client.get(Uri.parse(url + 'events'), headers: requestHeaders);
      final pEvents = parseEvents(response.body);
      return pEvents;
    } catch (e) {
      rethrow;
    } finally {
  ...
  Future<Event> getEvent(int id) async {
    ...
    try {
      var response =
          await client.get(Uri.parse(url + 'events' + '/' + id.toString()));
      var parsed = jsonDecode(response.body);
  ...
  Future<List<Report>> getReports(int uid) async {
    ...
    try {
      var response = await client
          .get(Uri.parse(url + 'users/' + uid.toString() + "/reports"));
  ...
  Future<Report> getReport(int id) async {
    ...
    try {
      var response = await client
          .get(Uri.parse(url + 'reports' + '/' + id.toString() + '/'));
      var parsed = jsonDecode(response.body);
  ...
  Future<Report> updateReport(Report r) async {
    ...
    try {
      response = await client.put(
        Uri.parse(url + 'reports/' + r.id.toString() + '/'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode(r),
      );
    } catch (e) {
  ...
  Future<User> getUser(int id) async {
    ...
    try {
      var response =
          await client.get(Uri.parse(url + 'users' + '/' + id.toString()));
      var parsed = jsonDecode(response.body);
  ...
  Future<List<User>> getRegistered(int id) async {
    ...
    try {
      response = await client.get(
          Uri.parse(url + 'events/' + id.toString() + '/register'),
          headers: {'Content-Type': 'application/json'});
    } catch (e) {
      print(e.errMsg());
    } finally {
  ...
  Future<bool> postRegistration(int eid, int uid) async {
    ...
    try {
      response = await client.post(
        Uri.parse(
            url + 'events/' + eid.toString() + '/register/' + uid.toString()),
        headers: {'Content-Type': 'application/json'},
      );
    } catch (e) {
      rethrow;
  ...
  // Maybe not needed or only for testing purposes
  // Future<User> getCurrentUser() async {
  //   return users[0];
  // }

  @override
  Future<User> addInterestAssociation(User u, String i) async {
    u.interests.add(i);
    this.updateUser(u);
  }
  ...
  // Possibly no longer needed as interests are stored in user object itself
  // @override
